#import shader_playground::my_pbr_bindings
#import shader_playground::noise
...
fn fragment(in: FragmentInput) -> @location(0) vec4<f32> {
    let pbr_result = pbr_fun(in);

    let pos = in.world_position.xz;
    let a = noise(pos - globals.time * material.distort_speed) * material.distortion_amount;
    let radius = material.radius;
    let distance_from_center = distance(vec2(0.0, 0.0), pos + a);
    let ring_mask = step(radius - material.ring_thickness, distance_from_center);
    let ring = material.ring_color * material.ring_glow_amount * ring_mask;
    let inverse_ring = (1.0 - ring_mask) * pbr_result;
    var color = (inverse_ring + ring);
...
fn pbr_fun(in: FragmentInput) -> vec4<f32> {
